...
public final class ReefTargetSelector {
    ...
    public static Optional<Pose2d> selectBestReefTarget(
        ...
        return visibleTags.stream()
                .filter(tag -> validTags.contains(tag.getId()))
                .min(Comparator.comparingDouble(
                        tag -> score(robotPose, tag)))
                .map(ReefTargetSelector::computeAlignPose);
    }

    private static double score(Pose2d robotPose, VisionTag tag) {
        ...
        return distance + headingError * 0.75;
    }
    ...
    public static Translation2d getReefNormal(Pose2d reefPose) {
        return new Translation2d(
                Math.cos(reefPose.getRotation().getRadians()),
                Math.sin(reefPose.getRotation().getRadians())
        );
    }
}
